...
2. Asks Claude whether the case is in one of your areas of interest:
...
Cost estimate: with claude-haiku-4-5, classifying 1,000 court items costs
well under €1. The all-courts feed publishes roughly 1–10 items per day.
...
You can also flip the order: require `match=True` AND `media_signal=True`
for lower-instance items.

**Too few hits** → check the Action logs for classifier reasoning;
loosen specific categories in the prompt.
